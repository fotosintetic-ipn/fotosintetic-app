...
func _on_device_connection_send_pressed():
	...
	var password = $device_connection/form/password.text
	
	if ssid == "" || password == "":
		return
	
	$device_connection/request.request(oxim_url + "/connect?ssid=" + ssid + "&password=" + password, PackedStringArray(), HTTPClient.METHOD_POST)

func _on_device_connection_request_completed(result, response_code, headers, body):
...
func _on_device_credentials_request_completed(result, response_code, headers, body):
	if response_code != 200:
